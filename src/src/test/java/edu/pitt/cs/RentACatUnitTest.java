...
public class RentACatUnitTest {
	...
	public void setUp() throws Exception {
		...
		c1 = CatSubmissionSubmission.createInstance(InstanceType.MOCK, 1, "Jennyanydots");
		c2 = CatSubmissionSubmission.createInstance(InstanceType.MOCK, 2, "Old Deuteronomy");
		c3 = CatSubmissionSubmission.createInstance(InstanceType.MOCK, 3, "Mistoffelees");
		...
		when(c3.getName()).thenReturn("Mistoffeless");
		when(c3.toString()).thenReturn("ID 3. Mistoffeless");
    ...
    doNothing().when(r).addCat(c1);
    doNothing().when(r).addCat(c2);
    doNothing().when(r).addCat(c3);
    
    String expectedList = "ID 1. Jennyanydots\nID 2. Old Deuteronomy\nID 3. Mistoffelees\n";
    when(r.listCats()).thenReturn(expectedList); 
    assertEquals(2, r.listCats());
	verify(r).addCat(c2);
}
    ...
    @Test
        ...
        when(r.listCats()).thenReturn(expected);
	...
	public void testRenameNumCat3() {
		...
		assumeTrue(r.renameCat(2, "Garfield"));
